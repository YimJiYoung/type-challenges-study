...
type ParsePrintFormat<Text> = Text extends `${infer Left}%${infer Key}${infer Right}`
  ? Key extends keyof ControlsMap
    ? [ControlsMap[Key], ...ParsePrintFormat<`${Left}${Right}`>]
    : ParsePrintFormat<`${Left}${Right}`>
  : [];
